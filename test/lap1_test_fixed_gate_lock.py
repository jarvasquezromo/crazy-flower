...
2. Gate detection (`_detect_green_gate`, `_gate_candidate`)
   - Threshold the (undistorted) image using HSV green colour and brightness,
     then morphologically close it.
...
3. State machine (`_send_setpoint`, runs at 10 Hz)
   ...
   SEARCH   -> yaw slowly in place until a gate is detected; collect several
               consistent world-frame observations before locking its position,
               then build a straight fly-through trajectory and switch to CHASE.
...
# Do not switch to CHASE from a single noisy frame.  While SEARCHing, collect
# a few recent world-frame estimates and lock only when they agree spatially.
...
PNP_LATERAL_MAX          = 3.00    # metres; reject impossible lateral/vertical PnP offsets
...
def _detect_green_gate(rgb_img):
    ...
    1. Threshold the image in HSV for green pixels, with an optional brightness
       floor to reject dark green/noisy background pixels.
    2. Morphologically close the mask, then validate contours as gate-shaped
       polygons (vertex count, aspect ratio, solidity). The centre is the
       polygon centroid, not the bbox.
    ...
    # The old code used grayscale brightness only, which could select any bright
    # object. This actually uses the configured green hue/saturation/value range.
    ...
    # Selection policy: take the most-right gate (largest centroid x), keeping
    # your original lap direction behaviour.
...
class FPVWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self._gate_obs   = []     # recent [(timestamp, (gx, gy, gz)), ...] before locking
    ...
    def _reset_gate_lock_buffer(self):
        """Clear uncommitted SEARCH observations."""
        self._gate_obs = []

    def _try_lock_gate_from_observations(self, gw, now):
        """Return a stable median gate estimate, or None if not confident yet.

        This is not full triangulation. It is multi-frame PnP filtering: each
        frame gives one gate-world estimate, and we only lock when the recent
        estimates agree within GATE_LOCK_MAX_SPREAD_XY.
        """
        if gw is None:
            return None
        ...
        self._gate_obs = [
            (t, p) for (t, p) in self._gate_obs
            if (now - t) <= GATE_LOCK_MAX_AGE_S
        ]
        self._gate_obs.append((now, (float(g[0]), float(g[1]), float(g[2]))))
        self._gate_obs = self._gate_obs[-GATE_LOCK_MAX_OBS:]

        if len(self._gate_obs) < GATE_LOCK_MIN_OBS:
            return None

        pts = np.asarray([p for _, p in self._gate_obs], dtype=np.float64)
        med = np.median(pts, axis=0)
        xy_spread = float(np.max(np.linalg.norm(pts[:, :2] - med[:2], axis=1)))
        z_spread = float(np.max(np.abs(pts[:, 2] - med[2])))

        # Require consistency in the floor plane. Height is also checked lightly
        # because PnP corner noise can create bad z estimates.
        if xy_spread <= GATE_LOCK_MAX_SPREAD_XY and z_spread <= 0.35:
            print(
                f"Locked gate from {len(self._gate_obs)} obs: "
                f"x={med[0]:.2f}, y={med[1]:.2f}, z={med[2]:.2f}, "
                f"spread_xy={xy_spread:.2f}, spread_z={z_spread:.2f}"
            )
    ...
    def _valid_chase_update(self, new_gw):
        """Reject sudden impossible gate jumps while already chasing."""
        if self._gate_world is None or new_gw is None:
            return False
        new_g = np.asarray(new_gw, dtype=np.float64).reshape(3)
        old_g = np.asarray(self._gate_world, dtype=np.float64).reshape(3)
        if not np.all(np.isfinite(new_g)):
            return False
        jump_xy = float(np.linalg.norm(new_g[:2] - old_g[:2]))
        jump_z = float(abs(new_g[2] - old_g[2]))
        return jump_xy <= CHASE_UPDATE_MAX_JUMP_XY and jump_z <= 0.50
    ...
    def _send_setpoint(self):
        ...
        if self._gate_state == "TAKEOFF":
            ...
            if found and bbox is not None:
                ...
                locked = self._try_lock_gate_from_observations(gw, now)
                if locked is not None:
                    ...
                    self._state_t0 = now
                    self._reset_gate_lock_buffer()
            else:
                # If the gate disappears during search, do not keep mixing old
                # observations with a later, possibly different detection.
                self._reset_gate_lock_buffer()
    ...
    def _gate_to_world(self, ex, ey, bbox, corners=None):
        ...
        if cam is not None:
            ...
            bw = bbox[2]
            dist = float(np.clip((GATE_PHYS_W * CAMERA_FX) / max(bw, 1), PNP_RANGE_MIN, PNP_RANGE_MAX))
